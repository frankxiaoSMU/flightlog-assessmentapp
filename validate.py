...
def validate_password(password: str):
    ...
    reg = r"\b^(?=.*[a-z])(?=.*[A-Z])[A-Za-z\d@$!#%*?&]{8,20}$\b"
...
def validate_flightLog(**args):
    """Validator"""
    if not args.get('tailNumber') or not args.get('flightID') or not args.get('takeoff') or not args.get('landing') or not args.get('Duration'):
        return "Key in all required fields"
    return True
